...
vector3 Engine::MonteCarloImportanceSampling(vector3& normale, double nPhong){
	...
	double phi = rand() * 2 * PI;//composante phi aléatoire
	double theta = acos(pow(rand(), 1/(nPhong+1)));//composante theta aléatoire avec prise en compte indice de phong
...
// Engine::Raytrace
// -----------------------------------------------------------
Couleur Engine::Raytracer( Ray& a_Ray, int a_Depth){
...
Couleur Engine::directeIllumination(Ray& ray,Intersection& intersection,Reflectance refl)
{
	...
	for (unsigned int i = 0; i < lumiere.size(); i++) {
		...
		Ray shadowRay = lumiere[i]->getShadowRay(intersection.getPoint(),ray);
...
bool Engine::intersectShadowRay(Ray& ray) {
	Intersection rec;
...
bool Engine::calculRefraction(Ray& ray, Intersection& intersection, Ray& refrRay, double& schlick){
	double n, nt;
